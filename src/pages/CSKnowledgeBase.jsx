...
export default function CSKnowledgeBase({ user }) {
  ...
  const { data: articles = [], isLoading } = useQuery({
    queryKey: ['kb-articles-cs'],
    queryFn: async () => {
      const allArticles = await base44.entities.KnowledgeBase.list('-created_date');
      return allArticles.filter(a => a.departments?.includes('customer_service'));
    }
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (newArticleData) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['kb-articles-cs'] });
      setShowEditor(false);
      setNewArticle({ title: '', content: '', category: 'general', departments: ['customer_service'] });
    }
  });
    ...
    <div className="relative">
      {/* Header */}
       <div className="mb-6 flex items-center justify-between gap-4">
         ...
         <Button
          onClick={() => openEditor()}
          className="h-12 bg-cyan-600 hover:bg-cyan-700"
        >
          ...
          filteredArticles.map(article => (
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-3">
                <h3 className="font-semibold text-slate-900 text-xl group-hover:text-cyan-600 transition-colors flex-1">
                  {article.title}
                </h3>
             ...
             <div className="p-8 max-w-3xl mx-auto">
               {showHistory ? (
                 ...
                 <>
                   ...
                   <div 
                     className="prose prose-slate prose-lg max-w-none prose-headings:font-bold prose-headings:text-slate-900 prose-p:text-slate-700 prose-p:leading-relaxed prose-a:text-cyan-600 prose-a:no-underline hover:prose-a:underline prose-img:rounded-xl prose-img:shadow-md prose-img:border-0 prose-strong:text-slate-900 prose-code:text-cyan-600 prose-code:bg-cyan-50 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-pre:bg-slate-900 prose-pre:shadow-lg prose-blockquote:border-l-4 prose-blockquote:border-cyan-500 prose-blockquote:bg-cyan-50 prose-blockquote:py-1"
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            ...
            <div>
             ...
             >
               {isDragging && (
                 <div className="absolute inset-0 z-10 bg-cyan-50/90 border-2 border-dashed border-cyan-400 rounded-lg flex items-center justify-center pointer-events-none">
                   <div className="text-center">
                     <Upload className="w-12 h-12 text-cyan-600 mx-auto mb-2" />
                     <p className="text-cyan-700 font-medium">Drop images here</p>
                   </div>
                 </div>
               )}
               <ReactQuill
