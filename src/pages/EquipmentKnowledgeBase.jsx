...
export default function EquipmentKnowledgeBase({ user }) {
  ...
  const { data: articles = [], isLoading } = useQuery({
    queryKey: ['kb-articles-equipment'],
    queryFn: async () => {
      const allArticles = await base44.entities.KnowledgeBase.list('-created_date');
      return allArticles.filter(a => a.departments?.includes('equipment'));
    }
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (newArticleData) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['kb-articles-equipment'] });
      setShowEditor(false);
      setNewArticle({ title: '', content: '', category: 'general', departments: ['equipment'] });
    }
  });
          ...
          filteredArticles.map(article => (
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-3">
                <h3 className="font-semibold text-slate-900 text-xl group-hover:text-orange-600 transition-colors flex-1">
                  {article.title}
                </h3>
              ...
              <div className="p-8 max-w-3xl mx-auto">
                {showHistory ? (
                  ...
                  <>
                    ...
                    <div 
                      className="prose prose-slate prose-lg max-w-none prose-headings:font-bold prose-headings:text-slate-900 prose-p:text-slate-700 prose-p:leading-relaxed prose-a:text-orange-600 prose-a:no-underline hover:prose-a:underline prose-img:rounded-xl prose-img:shadow-md prose-img:border-0 prose-strong:text-slate-900 prose-code:text-orange-600 prose-code:bg-orange-50 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-pre:bg-slate-900 prose-pre:shadow-lg prose-blockquote:border-l-4 prose-blockquote:border-orange-500 prose-blockquote:bg-orange-50 prose-blockquote:py-1"
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            ...
            <div>
              ...
              >
                {isDragging && (
                  <div className="absolute inset-0 z-10 bg-orange-50/90 border-2 border-dashed border-orange-400 rounded-lg flex items-center justify-center pointer-events-none">
                    <div className="text-center">
                      <Upload className="w-12 h-12 text-orange-600 mx-auto mb-2" />
                      <p className="text-orange-700 font-medium">Drop images here</p>
                    </div>
                  </div>
                )}
                <ReactQuill
