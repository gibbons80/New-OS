...
export default function ScheduleKnowledgeBase({ user }) {
  ...
  const { data: articles = [], isLoading } = useQuery({
    queryKey: ['kb-articles-schedule'],
    queryFn: async () => {
      const allArticles = await base44.entities.KnowledgeBase.list('-created_date');
      return allArticles.filter(a => a.departments?.includes('schedule'));
    }
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (newArticleData) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['kb-articles-schedule'] });
      setShowEditor(false);
      setNewArticle({ title: '', content: '', category: 'general', departments: ['schedule'] });
    }
  });
          ...
          filteredArticles.map(article => (
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-3">
                <h3 className="font-semibold text-slate-900 text-xl group-hover:text-fuchsia-600 transition-colors flex-1">
                  {article.title}
                </h3>
              ...
              <div className="sticky top-0 bg-white/90 backdrop-blur-lg border-b border-slate-200/80 px-8 py-5 flex items-center justify-between z-10">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    className="hover:bg-fuchsia-50 hover:text-fuchsia-700 hover:border-fuchsia-300"
                  >
              ...
              <div className="p-8 max-w-3xl mx-auto">
                {showHistory ? (
                      ...
                      {articleHistory.map((entry, index) => (
                        <div key={entry.id} className="bg-white border border-slate-200 rounded-xl p-5">
                          <div className="flex items-start justify-between gap-4 mb-3">
                            ...
                            {index === 0 && (
                              <span className="px-2 py-1 bg-fuchsia-100 text-fuchsia-700 text-xs font-medium rounded-full">
                                Latest
                              </span>
                            )}
                          </div>
                  ...
                  <>
                    ...
                    <div 
                      className="prose prose-slate prose-lg max-w-none prose-headings:font-bold prose-headings:text-slate-900 prose-p:text-slate-700 prose-p:leading-relaxed prose-a:text-fuchsia-600 prose-a:no-underline hover:prose-a:underline prose-img:rounded-xl prose-img:shadow-md prose-img:border-0 prose-strong:text-slate-900 prose-code:text-fuchsia-600 prose-code:bg-fuchsia-50 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-pre:bg-slate-900 prose-pre:shadow-lg prose-blockquote:border-l-4 prose-blockquote:border-fuchsia-500 prose-blockquote:bg-fuchsia-50 prose-blockquote:py-1"
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            ...
            <div>
              ...
              >
                {isDragging && (
                  <div className="absolute inset-0 z-10 bg-fuchsia-50/90 border-2 border-dashed border-fuchsia-400 rounded-lg flex items-center justify-center pointer-events-none">
                    <div className="text-center">
                      <Upload className="w-12 h-12 text-fuchsia-600 mx-auto mb-2" />
                      <p className="text-fuchsia-700 font-medium">Drop images here</p>
                    </div>
                  </div>
                )}
                <ReactQuill
