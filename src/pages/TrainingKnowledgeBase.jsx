...
export default function TrainingKnowledgeBase({ user }) {
  ...
  const { data: articles = [], isLoading } = useQuery({
    queryKey: ['kb-articles-training'],
    queryFn: async () => {
      const allArticles = await base44.entities.KnowledgeBase.list('-created_date');
      return allArticles.filter(a => a.departments?.includes('training'));
    }
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (newArticleData) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['kb-articles-training'] });
      setShowEditor(false);
      setNewArticle({ title: '', content: '', category: 'general', departments: ['training'] });
    }
  });
          ...
          filteredArticles.map(article => (
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-3">
                <h3 className="font-semibold text-slate-900 text-xl group-hover:text-indigo-600 transition-colors flex-1">
                  {article.title}
                </h3>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            ...
            <div>
             ...
             >
               {isDragging && (
                 <div className="absolute inset-0 z-10 bg-indigo-50/90 border-2 border-dashed border-indigo-400 rounded-lg flex items-center justify-center pointer-events-none">
                   <div className="text-center">
                     <Upload className="w-12 h-12 text-indigo-600 mx-auto mb-2" />
                     <p className="text-indigo-700 font-medium">Drop images here</p>
                   </div>
                 </div>
               )}
               <ReactQuill
